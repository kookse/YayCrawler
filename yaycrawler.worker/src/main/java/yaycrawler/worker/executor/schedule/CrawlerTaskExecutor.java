...
public class CrawlerTaskExecutor implements IScheduleTaskDealMulti<CrawlerTask> {
    ...
    @Override
    public boolean execute(CrawlerTask[] tasks, String ownSign) throws Exception {
        logger.info("采集任务开始执行...");
        logger.debug("--------------------------parsetask start-----------------------------------------");
        for (CrawlerTask task:tasks) {
            logger.debug(task.getId() + "*****" + task.getCode());
        }
    ...
    @Override
    public List<CrawlerTask> selectTasks(String taskParameter, String ownSign, int taskItemNum, List<TaskItemDefine> taskItemList, int eachFetchDataNum) throws Exception {

        List<CrawlerTask> configs = crawlerTaskExecutionPuller.select(ImmutableMap.of("taskItemList",taskItemList,"taskItemNum",taskItemNum), null, 5L);
    ...
    @Override
    public Comparator<CrawlerTask> getComparator() {
        return Comparator.comparing(CrawlerTask::getId);
    }
}
