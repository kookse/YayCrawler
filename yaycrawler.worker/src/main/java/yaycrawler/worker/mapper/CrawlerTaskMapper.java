...
public interface CrawlerTaskMapper {
    ...
    @Select("<script>" +
            ...
            " from crawler_task " +
            " where status = #{status} and mod(o_t.id,#{taskItemNum}) in " +
    ...
    @Update("update crawler_task set status=#{status},message=#{msg} where code=#{code}")
    int updateCrawlerTaskStatus(@Param("code") String code, @Param("status") int status, @Param("msg") String msg);
}
