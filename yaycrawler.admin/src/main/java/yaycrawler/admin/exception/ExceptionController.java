...
import yaycrawler.common.model.WebResult;
...
public class ExceptionController {
    ...
    public Object handleAllException(HttpServletRequest request, Exception ex) {
        String message = ex.getMessage();
        logger.error(ex.getMessage(), ex);

        String accept = request.getHeader("accept");
        if (accept.contains("application/json")) {
            if (StringUtils.isBlank(message)) message = ex.toString();
            return new ResponseEntity<>(WebResult.failure(message), HttpStatus.OK);
        }
